...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def legalNeighbors: List[(Block, Move)] = {
    	def legalNeighborsIter(neighbors: List[(Block, Move)]): List[(Block, Move)] = {
    	  if (neighbors.isEmpty) Nil
    	  else if (neighbors.head._1.isLegal) neighbors.head :: legalNeighborsIter(neighbors.tail)
    	  else legalNeighborsIter(neighbors.tail)
    	}
    	legalNeighborsIter(neighbors)
    }
