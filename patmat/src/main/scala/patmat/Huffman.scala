...
object Huffman {
  ...
  // Part 1: Basics
  def subTree(tree: CodeTree, leftTree: Boolean): CodeTree = tree match {
    case Fork(left, right, _, _) => if (leftTree) left else right
  ...
  def chars(tree: CodeTree): List[Char] = tree match {
    case Fork(_, _, chars, _) => chars.toList
   ...
   */
  def until(repeat: List[CodeTree] => Boolean, 
		    transform: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    if (repeat(trees)) trees
    else until(repeat, transform)(transform(trees))
  }
